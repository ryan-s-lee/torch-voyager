...
class ModelWrapper:
    def __init__(self, config, opts) -> None:
        ...
        self.page_inverter = {v: k for k, v in self.data.unique_pages.items()}
    ...
    def train_step(
        ...
        if self.config["online"]:
            self.write_prediction((pg_pred, off_pred), sys.stdout.buffer)
    ...
    def print_stats(
        ...
        print(
            ...
            f"lr {self.scheduler.get_last_lr()}, ", file=sys.stderr
        )
    ...
    def infer(self) -> None:
        ...
        page_inverter = {v: k for k, v in self.data.unique_pages.items()}
        self.model.eval()
    ...
    def write_prediction(self, predictions, buffer):
        ...
        page_predicts, offset_predicts = [pred.reshape(-1).cpu() for pred in predictions]
